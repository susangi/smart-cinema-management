...
/**
 * Service layer for generating and exporting revenue reports.
 */
@Service
public class RevenueReportService {
    ...
    /**
     * Generates a RevenueReportDTO including totals and trend data.
     */
    public RevenueReportDTO generate(RevenueReportParams params) {
        ...
        var totals = rows.stream().collect(
                Collector.of(
                        ...
                        (acc, r) -> {
                            acc[0] = acc[0].add(r.sales());
                            acc[1] = acc[1].add(r.refunds());
                        },
    ...
    /**
     * Converts a RevenueReportDTO into CSV format bytes for download.
     */
